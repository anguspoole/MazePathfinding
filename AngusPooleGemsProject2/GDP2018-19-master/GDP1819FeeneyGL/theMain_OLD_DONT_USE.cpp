...
//	// Geometry 
	...
	geomShader.fileName = "geom01.glsl";
	geomShader.shaderType = cShaderManager::cShader::GEOMETRY_SHADER;
...
//
//			cMeshSceneObject* pBunny = findObjectByFriendlyName("Roger");
//
...
//		::g_pDalekMangerBuffer->getAllPositions(vecPositions);

		glm::vec3 dalekPossitions[::g_NUMBEROFDALEKS];
		::g_pDalekMangerBuffer->getAllPositions( dalekPossitions, ::g_NUMBEROFDALEKS );

		::g_pDalekMangerBuffer->SwitchBuffers();

		cMeshSceneObject* pDalek = findObjectByFriendlyName("Dalek");
		pDalek->bIsVisible = true;

		for ( unsigned int index = 0; index != ::g_NUMBEROFDALEKS; index++ )
//		for ( unsigned int index = 2; index != 3; index++ )
		{
			pDalek->position = dalekPossitions[index];
			pDalek->setMeshOrientationEulerAngles( glm::vec3( -90.0f, 0.0f, 0.0f), true );
			
			glm::mat4 matModelIdentity = glm::mat4(1.0f);
			DrawObject( pDalek, matModelIdentity, program );
		}
...
//		p2SidedQuad->setUniformScale( 1.0f );
		...
		matView = glm::lookAt( glm::vec3(0.0f, 0.0f, +5.0f),			// Eye
...
//	// Draw it again, but as wireframe (so we know where it was)
//	p2SidedQuad->b_HACK_UsesOffscreenFBO = false;
//	p2SidedQuad->bIsWireFrame = true;
//	p2SidedQuad->position.z = 19.98f;
//	//p2SidedQuad->bUseVertexColour = true;
//	//p2SidedQuad->materialDiffuse = glm::vec4(1.0f,1.0f,1.0f,1.0f);
//	sTextureInfo grassTest;
//	grassTest.name = "grass.bmp";
//	grassTest.strength = 1.0f;
//	p2SidedQuad->vecTextures.clear();
//	p2SidedQuad->vecTextures.push_back( sTextureInfo(grassTest) );
//	DrawObject( p2SidedQuad, matModel, program );
...
//		glBindFramebuffer( GL_FRAMEBUFFER, 0 );		// Point output to screen
//	
//		glUniform1f( renderPassNumber_UniLoc, 2.0f );	// Tell shader it's the 2nd pass
//	
//		// Point the pass1OutputTexture (in shader) to the 
//		// FBO colour buffer output
//		GLint pass1OutputTexture_UniLoc = 
//			glGetUniformLocation( program , "pass1OutputTexture" );		
//		// Pick a texture unit
//		const int FBO_TEXTURE_UNIT = 34;			// Why? Because I like the number
//		glActiveTexture( GL_TEXTURE0 + FBO_TEXTURE_UNIT );
//		// Connect this UNIT to the TEXTURE
//		glBindTexture( GL_TEXTURE_2D, g_FBO_colourTexture );
//		// Connect the shader sampler to this texture
//		glUniform1i( pass1OutputTexture_UniLoc, FBO_TEXTURE_UNIT );
//	
//		// Draw a single quad that fills the screen....

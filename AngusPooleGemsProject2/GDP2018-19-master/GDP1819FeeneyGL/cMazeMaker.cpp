	...
	for (unsigned int a = 0; a < this->maze.size(); a++) 
	{
		for (unsigned int b = 0; b < this->maze[a].size(); b++) 
		{
			if (this->maze[a][b][0]) 
			{
				...
				cMeshSceneObject* pWallCube = new cMeshSceneObject();
				pWallCube->setDiffuseColour(glm::vec3(0.0f, 0.0f, 0.0f));
				pWallCube->bDontLight = false;
				pWallCube->friendlyName = "cube" + std::to_string(a) + std::to_string(b);
				pWallCube->vecLODMeshs.push_back(sLODInfo("cube_flat_shaded_xyz_n_uv.ply"));
				pWallCube->uniformScale = 1.0f;
				pWallCube->position = glm::vec3(a * 2.001f, 0.0f, b * 2.001f);
				pWallCube->bIsWireFrame = false;
				//pWallCube->setAlphaTransparency(0.5f);

				pWallCube->pDebugRenderer = ::g_pDebugRenderer;
				...
				pWallCube->vecTextures.push_back(sTextureInfo(bricks));

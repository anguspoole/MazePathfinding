...
bool AreSpheresPenetrating( cMeshSceneObject* pA, cMeshSceneObject* pB );

bool TestForCollision( cMeshSceneObject* pA, cMeshSceneObject* pB );
					  ...
					  std::vector< cMeshSceneObject* > &vec_pObjectsToDraw )
{
	...
	for ( std::vector< cMeshSceneObject* >::iterator itMesh = vec_pObjectsToDraw.begin();
		  itMesh != vec_pObjectsToDraw.end(); itMesh++ )
	{
		cMeshSceneObject* pCurMesh = *itMesh;		 
...
//
	...
	// Test for collisions
	for ( std::vector< cMeshSceneObject* >::iterator itObjectA = vec_pObjectsToDraw.begin();
		itObjectA != vec_pObjectsToDraw.end(); itObjectA++ )
	{
		// Go through all the other objects and test with this one...

		for ( std::vector< cMeshSceneObject* >::iterator itObjectB = vec_pObjectsToDraw.begin();
				itObjectB != vec_pObjectsToDraw.end(); itObjectB++ )
		{

			cMeshSceneObject* pObjectA = *itObjectA;
			cMeshSceneObject* pObjectB = *itObjectB;
			// Same? 
			...
			if ( pObjectA != pObjectB )
			{
				     ...
				     ( pObjectB->pTheShape != NULL ) )
				{
					if ( TestForCollision( pObjectA, pObjectB ) )
					{
						...
						if ( (pObjectA->shapeType == cMeshSceneObject::SPHERE) && (pObjectB->shapeType == cMeshSceneObject::SPHERE) )
						{
						...
						else if ( (pObjectA->shapeType == cMeshSceneObject::SPHERE) && (pObjectB->shapeType == cMeshSceneObject::TRIANGLE) )
						{
...
//		sPlyVertex corner_1 = theMeshDrawInfo.pMeshData->pVertices[CurTri.vertex_index_1];
...
bool SphereTraingleTest( cMeshSceneObject* pSphere, cMeshSceneObject* pTriangle )
{
...
bool TestForCollision( cMeshSceneObject* pA, cMeshSceneObject* pB )
{
	...
	// Sphere - sphere
	if ( (pA->shapeType == cMeshSceneObject::SPHERE) && 
		 (pB->shapeType == cMeshSceneObject::SPHERE ) )
	{
		return AreSpheresPenetrating( pA, pB );
	}
	else if ( (pA->shapeType == cMeshSceneObject::SPHERE ) && 
			  (pB->shapeType == cMeshSceneObject::TRIANGLE ) )
	{

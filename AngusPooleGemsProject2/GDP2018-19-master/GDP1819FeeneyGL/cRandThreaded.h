...
template <class T>
T getRandInRange_RT( T min, T max )
{
	double value = 
...
// Inspired by: https://stackoverflow.com/questions/686353/c-random-float-number-generation
template <class T>
T getRandInRange_RT(T min, T max, T randValueFrom0to1)
{
...
public:
	cRandThreaded();
	~cRandThreaded();
	double getNextRandDouble_0_to_1(void);
	double getNextRandDoubleInRange( double min, double max );

	static const int SIZEOFBUFFER = 1000;

...
class cGameEntity;	// for friend declaration


class cPhysicalProperties //: public iGameObjectSetsID	
{
public:
	// You could make an interface class so this can 'get at' the private, unique ID, 
	//  but here, I'm just making this a friend class. 
	// (normally, I would say this is 'bad design', but that's because it's 'dangerous' - i.e. breaks encapsulation)
	// NOTE: This is so that cGameEntity can set THIS object, not the other way around.
	// (so the unique ID for this class can be the same as the game entity class)
	friend cGameEntity;	// to access m_GameObjectID
	...
	unsigned int getGameGameEntityUniqueID(void) { return this->m_GameEntityUniqueID; }
private:
	// To be able to match up to the game objects. Can be set by friend CGameObject
	unsigned int m_GameEntityUniqueID;	
};

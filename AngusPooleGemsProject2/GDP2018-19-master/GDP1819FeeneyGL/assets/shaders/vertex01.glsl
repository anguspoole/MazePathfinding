...
uniform float boneBlends[MAXNUMBEROFBONES];
...
//		mat4 bonePoses[4];
//		bonePoses[0] =   bonesPose1[ int(vBoneID[0]) ] * ( 1.0 - boneBlends[ int(vBoneID[0]) ] )
//		               + bonesPose2[ int(vBoneID[0]) ] * boneBlends[ int(vBoneID[0]) ];
//		bonePoses[1] =   bonesPose1[ int(vBoneID[1]) ] * ( 1.0 - boneBlends[ int(vBoneID[1]) ] )
//		               + bonesPose2[ int(vBoneID[1]) ] * boneBlends[ int(vBoneID[1]) ];
//		bonePoses[2] =   bonesPose1[ int(vBoneID[2]) ] * ( 1.0 - boneBlends[ int(vBoneID[2]) ] )
//		               + bonesPose2[ int(vBoneID[2]) ] * boneBlends[ int(vBoneID[2]) ];
//		bonePoses[3] =   bonesPose1[ int(vBoneID[3]) ] * ( 1.0 - boneBlends[ int(vBoneID[3]) ] )
//		               + bonesPose2[ int(vBoneID[3]) ] * boneBlends[ int(vBoneID[3]) ];
//   		
//		mat4 BoneTransform 
//		                = bonePoses[0] * vBoneWeight[0];
//		BoneTransform += bonePoses[1] * vBoneWeight[1];
//		BoneTransform += bonePoses[2] * vBoneWeight[2];
//		BoneTransform += bonePoses[3] * vBoneWeight[3];
...
//		mat4 matrixWorldBone = BoneTransform * matModel;
//		
...
//		gl_Position = MVP * vec4(posTemp.xyz, 1.0);			// ON SCREEN
		
//		vsOuput.vertPosWorld = matModel * vec4(posTemp, 1.0);
		vsOuput.vertPosWorld = matModel * vec4(vertPositionFromBones.xyz, 1.0);
...
//		mat4 matBoneTransform_InTrans = inverse(transpose( (BoneTransform1 * boneBlend) + 
//		                                                   (BoneTransform2 * (1.0f - boneBlend) ) ) );

...
//		vec3 ObjectNormal = texture( texObjectColour, vertUV_x2.st ).rgb;
		...
		//finalOutputColour.rgb = texture( texPass1OutputTexture, gsOuput.vertUV_x2.st ).rgb;
		...
		finalOutputColour.rgb = texture( texPass1OutputTexture, textCoords.st ).rgb;
...
//		finalOutputColour.rgb = vec3(bw,bw,bw);
	...
	materialDiffuse.xyz += ( gsOuput.vertTanXYZ.xyz * 0.001f);
	materialDiffuse.xyz += ( gsOuput.vertBiNormXYZ.xyz * 0.001f);
	...
	if ( bAddReflect )
	{
		...
		vec3 viewVector = eyeLocation.xyz - gsOuput.vertPosWorld.xyz;
		viewVector = normalize(viewVector); 
		
		// Calcualte the reflection vector
		vec3 vReflect = reflect(viewVector, gsOuput.vertNormal.xyz);
	...
	if (bAddRefract)
	{
		...
		// Calcualte the reflection vector
		vec3 vRefract = refract(viewVector, gsOuput.vertNormal.xyz, 1.10);
...
//theLights[0].atten;
	...
	vec3 norm = normalize(gsOuput.vertNormal.xyz);
	...
	{	
		...
		vec3 vLightToVertex = theLights[index].position.xyz - gsOuput.vertPosWorld.xyz;
		...
		{	
			...
			vec3 vertexToLight = gsOuput.vertPosWorld.xyz - theLights[index].position.xyz;
...
//	finalOutputColour.rgb *= 1.25f;
	...
	if ( bIsParticleImposter ) 
	{
		...
		vec4 tex0Col = texture( texture00, gsOuput.vertUV_x2.st ).rgba;	// Imposter texture (smoke, fire)
